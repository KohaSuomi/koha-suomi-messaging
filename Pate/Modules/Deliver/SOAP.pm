...
sub callSOAPSigner {
    ...
    my $trustfile=C4::Context->config('ksmessaging')->{'suomifi'}->{'branches'}->{"$param{'branchconfig'}"}->{'wsapi'}{'sign'}->{'trust'};
    my $keyfile=C4::Context->config('ksmessaging')->{'suomifi'}->{'branches'}->{"$param{'branchconfig'}"}->{'wsapi'}->{'sign'}->{'key'};
    ...
    system ( "java -cp Modules/Deliver/Signer/signsoap.jar signsoap.soap_main $keyfile $trustfile $nullstderr &" );
